...
public class NotificationUtils {
  ...
  private static String formatEventReminderLong(VEvent event, Context c) {
    return String.format(c.getString(R.string.notification_long), formatTopic(event, c),
        formatTime(event, c), formatRoom(event, c));
  }

  public static String formatEventReminderShort(VEvent event, Context c) {
    return String.format(c.getString(R.string.notification_short), formatTime(event, c),
        formatRoom(event, c));
  }

  private static String formatTime(VEvent event, Context c) {
    Dur d = new Dur(event.getStartDate().getDate(), event.getEndDate().getDate());
    Date time_start = CalenderUtils
        .getNextRecuringStartDate(event, CalenderUtils.dateWithOutTime(event));
    ...
    return String.format(c.getString(R.string.notification_time), sdf.format(time_start),
        sdf.format(time_end));
  }
  ...
  private static int showNotification(String title, String textShort, String textLong,
      Context context) {
    ...
    NotificationCompat.Builder builder = new NotificationCompat.Builder(context,
        Constants.NOTIFICATION_CHANNEL_ALARMS_ID)
    ...
    builder.setSmallIcon(R.drawable.ic_round_github);
  ...
  private static void createNotificationChannel(Context context, boolean vibrate) {
    ...
    NotificationChannel mChannel = new NotificationChannel(Constants.NOTIFICATION_CHANNEL_ALARMS_ID,
        name, importance);
