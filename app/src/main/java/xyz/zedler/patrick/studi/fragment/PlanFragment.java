...
import xyz.zedler.patrick.studi.databinding.FragmentPlanBinding;
...
public class PlanFragment extends BaseFragment implements OnClickListener {
  ...
  public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
    ...
    systemBarBehavior.setAppBar(binding.appBarPlan);
    systemBarBehavior.setContainer(binding.framePlanContainer);
    systemBarBehavior.setUp();

    new ScrollBehavior(activity).setUpScroll(binding.appBarPlan, null, true);

    binding.toolbarPlan.setOnMenuItemClickListener(item -> {
  ...
  public void onClick(View v) {
    int id = v.getId();
    if (id == R.id.button_plan_download && getViewUtil().isClickEnabled()) {
      performHapticClick();
